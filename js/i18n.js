(function () {
  ...
  var I18N = {
    'pt-BR': {
      ...
      project_orderservice_short: 'Pipeline CI/CD completo com GitHub Actions, deploy automático em Kubernetes (k3s) na GCP. Inclui build multi-stage Docker, scan de vulnerabilidades com Trivy, métricas Prometheus e PostgreSQL com StatefulSet.',
      project_orderservice_full: 'Pipeline CI/CD completo construído com GitHub Actions, com deploy automático em Kubernetes (k3s) na Google Cloud Platform. Inclui build multi-stage Docker para otimização do tamanho da imagem, scan de vulnerabilidades com Trivy integrado ao pipeline, coleta de métricas com Prometheus e banco de dados PostgreSQL como StatefulSet. Todo o fluxo, do push de código até a atualização dos pods em produção, é totalmente automatizado.',
      project_youtube_short: 'Aplicação Streamlit que resume automaticamente vídeos do YouTube extraindo legendas e processando via API Mistral AI. Suporta modo de interpretação de músicas, detecção automática de idioma e exportação em TXT/PDF.',
      project_youtube_full: 'Aplicação web construída com Streamlit que resume automaticamente vídeos do YouTube. Extrai as legendas do vídeo e processa o texto via API Mistral AI para gerar um resumo coerente. Possui modo especial de interpretação de músicas, detecção automática de idioma e exportação dos resumos em TXT ou PDF. Interface simples e direta, ideal para consumir conteúdo longo em fração do tempo.',
    ...
    'en-US': {
      ...
      project_orderservice_short: 'End-to-end CI/CD pipeline with GitHub Actions and automatic Kubernetes (k3s) deploy on GCP. Includes multi-stage Docker build, Trivy vulnerability scanning, Prometheus metrics, and PostgreSQL StatefulSet.',
      project_orderservice_full: 'Complete CI/CD pipeline built with GitHub Actions, with automatic deployment to Kubernetes (k3s) on Google Cloud Platform. Includes multi-stage Docker build to optimize image size, Trivy vulnerability scanning integrated into the pipeline, Prometheus metrics collection, and PostgreSQL as a StatefulSet. The full flow, from code push to pod updates in production, is fully automated.',
      project_youtube_short: 'Streamlit app that automatically summarizes YouTube videos by extracting captions and processing them with the Mistral AI API. Supports song interpretation mode, automatic language detection, and TXT/PDF export.',
      project_youtube_full: 'Web app built with Streamlit that automatically summarizes YouTube videos. It extracts video captions and processes the text through the Mistral AI API to generate a coherent summary. It includes a special song interpretation mode, automatic language detection, and TXT/PDF export. Clean and direct interface, ideal for consuming long content in a fraction of the time.',
